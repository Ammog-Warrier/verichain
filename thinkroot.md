...
### 5.5 Verifiable Credential API (VC API) [New]

| Method | Function           | Endpoint                       | Payload           | Response                                     |
| ------ | ------------------ | ------------------------------ | ----------------- | -------------------------------------------- |
| GET    | `getCredential`    | `/credentials/:batchId`        | -                 | `{ batchId, vc: { ... }, issuer, issuedAt }` |
| POST   | `generateVCQRCode` | `/credentials/:batchId/qrcode` | `{ format?: 'svg' | 'png' }`                                     |
| GET    | `exportCredential` | `/credentials/:batchId/export` | -                 | `{"batchId":..., "vc":{...}}`                |
...
* **Features:**
  ...
  * **Verifiable Credential View (VC View):** New and integrated feature to view verifiable credentials associated with the Batch ID.

    * Access: Visible on Public Verify page when a VC is available for the Batch ID or via a dedicated VC sub-section.

    * Features:

      * VC Summary Card showing issuer, issuance date, and batchId.

      * "View Verifiable Credential" action that expands to show the full VC payload.

      * QR Code generation for the VC:

        * Generates a scannable QR code that encodes a verifiable credential URL or embedded VC data reference.

        * Button: "Generate VC QR Code" (automatically invoked if VC is present) or manual trigger.

        * QR Code is saved as an image, with options to download (PNG/SVG) and copy the data URL.

      * QR Code Display:

        * Inline rendered QR code for quick scanning.

        * Optional large view modal for scanning by mobile devices.

      * Interaction with VC:

        * Copy VC JSON (pretty-printed) to clipboard.

        * Export as JSON file (download) for offline verification.

        * Validate integrity: show status badge (Verified/Invalid) with timestamp.

      * Edge Cases:

        * If VC payload is missing, show a clear message: "Verifiable Credential not found for this Batch ID."

        * If VC encoding is too large, fallback to URL reference and show reduced payload view with a link.

### 6.5 Retailer Verification Model (Unified VC Access)

* For Retailer/Hospitals, the VC view can be accessed from the Retailer Terminal when an asset has an associated VC. The VC data is fetched via `GET /credentials/:batchId` and rendered in a read-only format with the same QC and export options.

### 6.6 Public VC Access Coordinated with Public Verify

* If a VC exists for a Batch ID in the public domain, the VC view is surfaced alongside the standard public verification results.
